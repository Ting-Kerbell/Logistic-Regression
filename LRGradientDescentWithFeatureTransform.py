...
class LRGDWithFeatureTransform(LogisticRegression):
    ...
    * utility methods like initialize_w
    ...
    def fit_feature_pipeline(self, x_NF, y_N=None):
        ...
        '''
        # caption and plug in a function 
        ...
        self.feature_transform_pipeline = sklearn.pipeline.Pipeline(
            [('feature_transform', sklearn.pipeline.FeatureUnion(transformer_list=[
                    ...
                    ('noGradient_x', FeatureRounder()),
                    #('numLight_x', LightNum()),
                    #('numGroups_x', GroupNum()),
                    #('cluster_x', FeatClus()),
                    ]),
            )])
...
class AverageValueFeatureExtractor(BaseEstimator, TransformerMixin):
    ...
    def fit(self, x, y=None):
        ...
        """
        return self
class FeatureRounder(BaseEstimator, TransformerMixin):
    ...
    def fit(self, x, y=None):
        ...
        """
        return self
class LightNum(BaseEstimator, TransformerMixin):
    ...
    def transform(self, x, y=None):
        ...
        feat : 2D array, size N x 1
            One feature extracted for each example
        """
        num_N = np.count_nonzero(x, axis = 1)[np.newaxis].T
        a = np.hstack([x, num_N])
        return a
    ...
    def fit(self, x, y=None):
        ...
        """
        return self
class GroupNum(BaseEstimator, TransformerMixin):

    def __init__(self):
        pass
    def getClusterNum(self, x_F):
        return measurements.label(x_F[:x_F.shape[0]].reshape(28, 28))[1]
    def transform(self, x, y=None):
        """ Average all feature values into a new feature column
        ...
        F = x.shape[1]
        x.reshape(N, F)
        d =np.apply_along_axis(self.getClusterNum, 1, x)[np.newaxis].T
        a = np.hstack([x, d])
        return a
    ...
    def fit(self, x, y=None):
        ...
        """
        return self
class FeatClus(BaseEstimator, TransformerMixin):
    ...
    def transform(self, x, y=None):
        ...
        F = x.shape[1]
        x.reshape(N, F)
        d =np.apply_along_axis(self.getClusterNum, 1, x)
        return d
...
if __name__ == '__main__':
    ...
    orig_lr = LogisticRegression(C=10.0)
    orig_lr.fit(x_N2, y_N)

    ## Run LR on transformed features!
    new_lr = LRGDWithFeatureTransform(C = 10.0)
    new_lr.fit(x_N2, y_N)
