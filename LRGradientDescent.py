...
class LogisticRegressionGradientDescent():
    ...
    * the extra L2 penalty term will only ever add to the loss
    ...
    def calc_z(self, x_NF, w_G):
        
        G = w_G.shape[0]
        F = G-1
        if F == x_NF.shape[1]:
            z_N = np.matmul(x_NF, w_G[:F])
            #z_N =  x_NF @ wt.T 
            z_N = z_N + w_G[-1]
            return z_N
        z_N =  np.matmul(x_NF, w_G)
        z_N = z_N[np.newaxis].T
        return z_N
        
        F = w_G.shape[0] -1
        w_F = w_G[:-1]
        bias = w_G[-1]
        z_N = np.matmul(x_NF[:, :F], w_F) + bias
        return z_N.flatten()
    def predict_proba_helper(self, x_NF, w_G):
        z_N = self.calc_z(x_NF, w_G)
    
        vectorized_sigma = np.vectorize(self.sigmoid_stable)
        yproba1_N1 = vectorized_sigma(z_N)
        
        yproba1_N1 = sigmoid_stable(z_N)
    ...
    def predict_proba(self, x_NF):
        ...
        w_F = self.w_G[:-1]
        bias = self.w_G[-1]
        z_N = np.matmul(x_NF, w_F) + bias
        '''
        vectorized_sigma = np.vectorize(self.sigmoid_stable)
       
        proba1_N = vectorized_sigma(z_N)
        '''
        proba1_N = sigmoid_stable(z_N)
        return np.hstack([1.0 - proba1_N[:,np.newaxis], proba1_N[:,np.newaxis]])
    ...
    def calc_loss(self, w_G, xbias_NG, y_N):
        ...
        '''
        N = float(y_N.size)
        z_N = np.matmul(xbias_NG, w_G)
        #z_N = self.calc_z(xbias_NG, w_G)
 
        denom = N * np.log(2)
        addedz = np.zeros((y_N.size, 2))
        addedz[:,1] = z_N.T
        log_loss = (-1)* np.subtract(np.matmul(y_N, z_N), np.sum(logsumexp(addedz, axis=1)))
        temp = np.matmul(w_G, w_G)
        L2_loss = 0.5 * self.alpha * temp 
      
        return np.add(L2_loss, log_loss) / denom
    ...
    def calc_grad(self, w_G, xbias_NG, y_N):
        ...
        grad_logloss_wrt_w_G = np.matmul((y_N - yproba1_N1), xbias_NG)
